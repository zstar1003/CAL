...
for image_file in tqdm(selected_images):
    ...
    source_label_path = os.path.join(source_labels_folder, image_file.replace('.jpg', '.txt'))  # 假设标签扩展名为.txt

    # 构造目标图片和标签的完整路径
    target_image_path = os.path.join(target_images_folder, image_file)
    target_label_path = os.path.join(target_labels_folder, image_file.replace('.jpg', '.txt'))
